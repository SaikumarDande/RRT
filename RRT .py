...
class RRTMap:
    ...
    def drawPath(self, n, map_animation):
        ...
        while(curr_n.parent != None):
            ...
            cv2.line(map_animation, (parent.x, parent.y), (curr_n.x, curr_n.y), self.Green, thickness = 1)
...
class RRTGraph:
    ...
    def add_node(self, new_node):
        ...
        if not self.map.crossObstacles(self.vertex[nnear_index], new_node, is_Goal):
            new_node.parent = self.vertex[nnear_index]
            self.vertex[nnear_index].childs.append(new_node)
            new_node.distance = self.vertex[nnear_index].distance + self.metric(self.vertex[nnear_index], new_node)
            if is_Goal:
